...
const Input = () => {
  ...
  const handleFilter = (event) => {
    ...
    const handlefiltre = customers
      .filter((customer) => customer.name.toLowerCase().includes(lowerSearch))
      .slice(0, 6);
    setFiltreCustomers(handlefiltre);
  };

  useEffect(() => {
    if (!lowerSearch || searchValue.length == 0) {
      setFiltreCustomers(recentsCustomers);
    }
  }, [searchValue]);
  ...
  return (
    <div className="input-container">
      ...
      <div ref={searchRef} className={`${search} search-box`}>
        {filtreCustomers.map((customer, index) => (
          <SearchInfo key={index} customer={customer} />
        ))}
      </div>
    </div>
  );
